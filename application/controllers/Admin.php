...
class Admin extends CI_Controller
{
    ...
    public function send_msg()
    {
        ...
        $url = "https://reguler.metakreasi.id/sms_api.php?action=kirim_sms&email=" . $email_api . "&passkey=" . $passkey_api . "&no_tujuan=" . $no_hp_tujuan . "&pesan=" . $isi_pesan;
    ...
    public function view_surat($no_surat)
    { //ga dipake
    ...
    public function tambah_notulensi($no_surat)
    { //upload notulensi
    ...
    public function tambah_ttd($no_surat)
    { //upload surat ttd
    ...
    public function tambah_hadir($no_surat)
    { //upload Daftar Hadir
    ...
    public function fpdf_hadir($no_surat)
    { //fpdf
        $this->load->library('myfpdf');
        $data = [
            'surat' => $this->Admin_model->getSuratWhere($no_surat)
    ...
    public function fpdf_notulen($no_surat)
    { //fpdf
        $this->load->library('myfpdf');
        $data = [
            'surat' => $this->Admin_model->getSuratWhere($no_surat)
    ...
    public function laporan_broadcast()
    {
        $this->form_validation->set_rules('tgl_awal', 'Tanggal Awal', 'required', [
            'required' => 'Tanggal Awal harus diisi!'
        ]);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view('templates/topbar');
            $this->load->view('templates/ad_sidebar');
            $this->load->view('admin/rept_broadcast', $data);
            $this->load->view('templates/footer');
        } else {
            $this->load->library('myfpdf');
            $data = [
                'awal' => $this->input->post('tgl_awal'),
                'akhir' => $this->input->post('tgl_akhir'),
                'broadcast' => $this->Admin_model->getBroadcastReport()
            ];
            $this->load->view('admin/fpdf_rpbroadcast', $data);
        }
    }
    ...
    public function tambah_bukti($id)
    { //upload surat ttd
